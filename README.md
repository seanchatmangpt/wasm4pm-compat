# wasm4pm-compat v26.6.15 — Process Intelligence Compatibility Core
...
The logical system version, target specification, and documented release standard for this codebase is `26.6.15`. All API behaviors, validation logic, and diagnostic receipts in this repository are designed to conform to the **`wasm4pm-compat v26.6.15`** standard. The root crate, workspace subcrates, and derived manifests are all natively configured to `26.6.15`.
...
| `nightly_foundry_surfaces` | (none) | `petri_law` (Marking EMPTY/total_tokens/at, PreMatrix enabling, PostMatrix firing — Murata 1989), `powl_law` (TypedNode<KIND> atom/silent/partial/xor/loop, PowlKind enum, OrderEdge concurrency — Kourani 2505.07052), `evidence_law` (EvidenceKind specialization raw→"raw"/Admitted→"admitted", zero-cost repr(transparent)), `token_law` (SIMD transition_enabled_4/8, fire_4 — Murata §2), `families_match_simd` (SIMD batch family bitmask) |
| `xes_interchange_grammar` | (none) | All 12 pub items in `src/xes.rs`: `CaseCentricMarker` (Display "case-centric", zero-sized), `XesExtension` (name/prefix/uri), `XesEvent` (builder chain, concept_name/timestamp/resource/lifecycle_transition/attributes), `XesTraceAttributes` (with/get/concept_name/len/is_empty), `XesTrace` (new/name/events/len/is_empty), `XesLog` (validate: ok + 6 named refusals), `XesToOcedProjectionShape` (standard/with_case_type/4 accessors), `XesDeclaredExtensionLaw` (NAME/REFUSAL_VARIANT/governs/description/Display), `XesExtensionPrefixWitness` (new/prefix/is_standard/standard_witnesses×4), `XesLifecycleTransition` (14 variants/as_str/parse), `XesStandardPrefix` (4 prefixes/as_str/parse), `XesRefusal` (10 named laws + Display) |
