...
use wasm4pm_compat::interop::{check_filter_shape, ConformanceTriple, FilterShape, Pm4pyShape, SummaryShape};
...
use wasm4pm_compat::prediction::{NextActivity, PredictionHorizon, PredictionProblem, PredictionTarget};
use wasm4pm_compat::process_cube::{CubeDimension, CubeDimensionKind, CubeSlice, ProcessCube};

fn main() {
    // Stage 1: Build an OCEL log
    let log = OcelLog::new(
        [Object::new("order-1", "order"), Object::new("item-1", "item")],
        [
    ...
    println!("Stage 1 ✓ OcelLog: {} objects, {} events", log.objects().len(), log.events().len());
    ...
    println!("Stage 4 ✓ PredictionProblem<NextActivity>: prefix={}, horizon={:?}", problem.prefix_len(), problem.horizon);
    ...
    println!("Stage 5 ✓ MultiPerspectiveEvidence<ControlFlow+Time>: inner.prefix_len={}", mp.inner.prefix_len());

    // Stage 6: Process cube slice
    let cube: ProcessCube<OcelLog, 2> = ProcessCube::new();
    assert_eq!(cube.dimension_count(), 2);
    let resource_slice: CubeSlice<CubeDimension<"resource">, &str> = CubeSlice { dimension: PhantomData, value: "warehouse-A" };
    let time_slice: CubeSlice<CubeDimension<"time">, &str> = CubeSlice { dimension: PhantomData, value: "2024-Q1" };
    ...
    println!("Stage 6 ✓ ProcessCube<OcelLog,2>: dims={}, slices=resource×time", cube.dimension_count());
