...
**Known issue (found 2026-08-24, unfixed): `Makefile.toml`'s `[tasks.ggen-*]` entries do not match the installed `ggen` CLI.** Verified against `ggen` v26.8.18: there is no `--manifest` and no `--rule` flag at all — `ggen sync --manifest X --rule Y` fails with `unexpected argument '--rule' found`. The real subcommand is `ggen sync run`, which reads `ggen.toml` from its current working directory only (no way to point it at a different manifest file, and it rejects `output_dir = ".."` as path traversal when invoked from `ggen/`). Every task in `Makefile.toml` lines 104–166 (`ggen-sync`, `ggen-witnesses`, `ggen-witnesses-dry`, and the six per-category witness tasks) currently calls the broken `--manifest`/`--rule` form and will fail if run as written. Confirmed workaround (used successfully this session to render `bindings/elixir/lib/wasm4pm_compat/plain_types.ex`): stage the target `.toml` as a temporary *root-level* `ggen.toml` with `output_dir = "."` and paths rewritten root-relative, run `ggen sync run` from the repo root, then remove the temp file. This is an external `ggen` CLI defect/version-drift issue, not something to patch in this repo's source — but do not assume `Makefile.toml`'s `ggen-*` tasks work until this is resolved (either by fixing the tasks to call `sync run` with only the flags v26.8.18 actually supports, or by the CLI itself being updated).

**Per-rule selection is not supported at all by the installed CLI** (re-confirmed 2026-08-24): `ggen sync run` has no way to pick just one `[[generation.rules]]` entry out of a `ggen.toml` — every rule the resolved manifest declares renders together on every invocation. The `cwd = "ggen"` already present in every `[tasks.ggen-*]` block is the *actual* mechanism that lets those tasks find `ggen/ggen.toml` at all (ggen resolves `ggen.toml` from cwd only, there is no manifest-path argument) — it is not incidental. The Makefile task names (`ggen-witnesses-cognition`, `ggen-witnesses-rdf`, etc.) are historical/aspirational per-rule naming the installed CLI's flags no longer back; e.g. `cargo make ggen-witnesses-dry` currently fails outright with `error: unexpected argument '--rule' found`. The working form for a dry run is `cd ggen && ggen sync run --dry-run` (renders every rule in `ggen/ggen.toml`, not just `witness-markers`).
