...
fn import_without_witness_or_fixture_reports_all_violations_at_once() {
    let mut b = ProcessBoundary::fully_attested(ProcessBoundaryKind::ImportsFormat, "ocel-in");
    b.has_witness = false;
...
fn conformance_claim_without_fields_is_named() {
    let mut b = ProcessBoundary::fully_attested(ProcessBoundaryKind::ClaimsConformance, "conf");
...
fn receipt_claim_without_shape_is_named() {
    let mut b = ProcessBoundary::fully_attested(ProcessBoundaryKind::ClaimsReceipt, "rcpt");
...
fn raw_evidence_export_is_refused_for_any_boundary() {
    let mut b = ProcessBoundary::fully_attested(ProcessBoundaryKind::EmitsEvents, "emitter");
...
fn violation_laws_are_specifically_named_strings() {
    ...
    assert_eq!(
        StrictViolation::MissingLossPolicy.law(),
        "MissingLossPolicy"
    );
    assert_eq!(
