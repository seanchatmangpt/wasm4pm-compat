...
| 3 | `Hierarchical Decomposition of Separable Workflow-Nets .pdf` | Hierarchical Decomposition of Separable Workflow-Nets | 2026 | Kourani, Park, van der Aalst | `POWL` | WF-net, separable WF-net, POWL 2.0, choice graph, partial order, state machine subclass, marked graph subclass | WF-net → POWL 2.0 transformation preserves language; separability is a structural law; choice graphs cover non-block-structured decision and cyclic logic | `src/powl.rs` has `TreeProjectable` sealed trait, `ChoiceGraphEdge`, `PowlNodeKind::ChoiceGraph`; `src/petri.rs` has `WfNetConst<SOUNDNESS>` and `SeparableWfNet<SOUNDNESS>` | Zero-cost: `SeparableWfNet<SOUNDNESS>` marker added; `ChoiceGraph` variant in `PowlNodeKind` | `tests/ui/compile_pass/separable_wfnet_marker.rs` | None | None | `WfNet2Powl` conversion witness still missing; compile-fail for non-separable conversion | `PARTIAL_WITH_REASON` — SeparableWfNet marker and ChoiceGraph type surface added; WfNet2Powl graduation bridge missing |
