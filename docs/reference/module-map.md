...
This document maps the public Rust module layout in `wasm4pm-compat` version `26.6.14` to their physical files. This mapping is compiled directly from the module declarations in `src/lib.rs`.
...
| **`parity`** | `src/parity/` | *None (Always-on)* | `MoveCategory`, `DeltaReport`, `DriftMonitor` |
...
| **`verifier`** | `src/verifier/` | *None (Always-on)* | `ResidualFailset`, `FailedTransition`, `DiagnosticPayload`, `ComplianceDiagnostic`, `VerificationReport` |
