...
This document provides a comprehensive overview of how `wasm4pm-compat` version `26.6.8` serves as the foundational process-evidence type standard for the entire process intelligence workspace.
...
To respect package and workspace constraints, the physical `Cargo.toml` version field is locked at `26.6.8`. However, the logical target of all reports, specifications, and documentation is the release version `26.6.8`.
